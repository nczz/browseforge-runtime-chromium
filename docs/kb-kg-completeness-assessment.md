...
- `WebGL`: configured vendor/renderer source patch and page-context evidence exist, and BrowserLeaks WebGL collection now records sanitized extension-count, extension hash, parameter hash, and bounded parameter summaries while flagging SwiftShader/Google software rendering; shader precision, extension-profile parity, rendered WebGL pixel output, and headed/native detector confirmation remain required before release-grade WebGL claims.
