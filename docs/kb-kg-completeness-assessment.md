...
| BrowseForge consumer contract | High | `release-gates.json` records BrowseForge commit `76ec639` dispatching `browseforge-chromium`; local dogfood evidence covers runtime config, profile create, session launch, runtime_id reporting, profile isolation, and Playwright Bind. | Keep adapter smoke evidence current as BrowseForge changes. |
...
- `AudioContext` and `fonts`: page-context and CreepJS/BrowserLeaks/Pixelscan bounded probes are recorded, and `detector-score-comparison.json` now compares CreepJS headless/headed audio deltas plus BrowserLeaks/CreepJS font glyph/metric hashes while explicitly listing the missing BrowserLeaks headed AudioContext summary counterpart, Pixelscan headed AudioContext/font availability counterparts, BrowserLeaks/Pixelscan score baselines, and native-headed font-corpus baseline gaps; release-grade BrowserLeaks/CreepJS/Pixelscan score baselines and platform corpus parity remain required.
- `WebGL`: configured vendor/renderer source patch and page-context evidence exist, and BrowserLeaks WebGL collection now records sanitized extension count/hash, bounded parameter hash, shader precision hash, rendered pixel hash, and pixel dimensions; `detector-score-comparison.json` now requires extension count plus all WebGL metadata hashes for every committed row and will report vendor/renderer, extension-profile, parameter, shader precision, and rendered-pixel match booleans when complete BrowserLeaks plus peer evidence exists. Legacy vendor-only rows still emit explicit metadata / cross-detector WebGL comparison gaps, while headed/native detector confirmation remains required before release-grade WebGL claims.
...
2. Add release-grade BrowserLeaks/CreepJS/Pixelscan score baselines for AudioContext and fonts, using `scripts/detector_harness.py collect --detector browserleaks --page audio|fonts`; BrowserLeaks audio/fonts page classifiers intentionally return warning-level evidence until committed score baselines and corpus comparisons exist.
