...
- `WebGL`: configured vendor/renderer source patch and page-context evidence exist, and BrowserLeaks WebGL collection now records sanitized extension hash, bounded parameter hash, shader precision hash, rendered pixel hash, and pixel dimensions; `detector-score-comparison.json` now emits explicit metadata-hash / cross-detector WebGL comparison gaps for legacy vendor-only rows, while extension-profile parity and headed/native detector confirmation remain required before release-grade WebGL claims.
