...
- `AudioContext` and `fonts`: page-context and CreepJS/BrowserLeaks metrics are recorded, and `detector-score-comparison.json` now compares CreepJS headless/headed audio deltas plus BrowserLeaks/CreepJS font glyph/metric hashes while explicitly listing the missing BrowserLeaks headed AudioContext summary counterpart, BrowserLeaks audio score baseline, Pixelscan audio/fonts baseline, and native-headed font-corpus baseline gaps; release-grade BrowserLeaks/CreepJS/Pixelscan score baselines and platform corpus parity remain required.
