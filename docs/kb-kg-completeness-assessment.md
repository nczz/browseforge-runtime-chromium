...
- `WebGL`: configured vendor/renderer source patch and page-context evidence exist, and BrowserLeaks WebGL collection now records sanitized extension-count, extension hash, parameter hash, shader precision hash, rendered pixel hash, and bounded parameter summaries while flagging SwiftShader/Google software rendering; extension-profile parity and headed/native cross-detector confirmation remain required before release-grade WebGL claims.
